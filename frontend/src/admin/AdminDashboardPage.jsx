...
import {
  Users, Wallet, TrendingUp, IndianRupee, Landmark, CheckCircle,
  XOctagon, Clock, UserCheck, ShieldCheck
} from "lucide-react";
...
const isTerminalWithdrawalStatus = (status) => terminalWithdrawalStatuses.includes(status);
...
export default function AdminDashboardPage() {
      ...
      setDashboard((prev) => {
        ...
        return {
          ...
          withdrawals: prev.withdrawals.map((withdrawal) => (
            withdrawal._id === data.withdrawal._id ? data.withdrawal : withdrawal
          )),
        };
      });
      setMessage(data.message || `Withdrawal marked as ${status}`);
    } catch (error) {
      setMessage(error.response?.data?.message || 'Could not update withdrawal');
    } finally {
  ...
  const WithdrawalConfirmIcon = pendingWithdrawalAction?.status === 'rejected' ? XOctagon : ShieldCheck;
  const isConfirmingWithdrawal = pendingWithdrawal && updatingWithdrawalId === pendingWithdrawal._id;
  ...
  const statCards = [
    { label: 'Total Users', value: summary.users || 0, icon: Users, bg: s.statBg1, c: s.statColor1 },
    { label: 'Paid Bookings', value: summary.paidBookings || 0, icon: CheckCircle, bg: s.statBg2, c: s.statColor2 },
    { label: 'Gross Revenue', value: formatMoney(summary.grossRevenue), icon: TrendingUp, bg: s.statBg3, c: s.statColor3 },
    { label: 'Platform Fees', value: formatMoney(summary.platformFees), icon: IndianRupee, bg: s.statBg4, c: s.statColor4 },
    { label: 'Provider Payouts', value: formatMoney(summary.providerPayouts), icon: Wallet, bg: s.statBg5, c: s.statColor5 },
    { label: 'Withdrawal Hold', value: formatMoney(summary.withdrawalHolds), icon: Landmark, bg: s.statBg6, c: s.statColor6 },
  ];
      ...
      <header className={s.header}>
        <div className={s.headerInner}>
          <div className={s.logoRow}>
            <img src={logo} alt="BookMe Admin" className={s.logoImg} />
          ...
          <div className={s.headerActions}>
            ...
            <button type="button" onClick={logout} className={s.logoutButton}>Log out</button>
          </div>
        </div>
      </header>
                ...
                <tbody className={s.tbody}>
                  {(dashboard?.users || []).map((user) => (
                    <tr key={user._id} className={s.tr}>
                      <td className={s.td}>
                        <div className={s.userBusinessName}>{user.businessName || user.name}</div>
                      </td>
                  ...
                  {dashboard && dashboard.users?.length === 0 && (
                    <tr><td colSpan="4" className={s.emptyTableCell}>No users found.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
            ...
            <div className={s.withdrawalList}>
              {(dashboard?.withdrawals || []).map((withdrawal) => {
                ...
                <div key={withdrawal._id} className={s.withdrawalItem}>
                  ...
                  <div className={s.withdrawalAccountInfo}>
                    ...
                    {withdrawal.payoutSnapshot?.bankName || 'UPI Connection'} • {withdrawal.payoutSnapshot?.accountLast4 ? `**** ${withdrawal.payoutSnapshot.accountLast4}` : withdrawal.payoutSnapshot?.upiId}
                  </div>
                    ...
                    {withdrawalStatuses.map((status) => {
                      ...
                      return (
                        <button
                          key={status}
                          type="button"
                          onClick={() => requestWithdrawalStatusChange(withdrawal, status)}
                          disabled={isUpdatingThisWithdrawal || isCurrentStatus || isWithdrawalLocked}
                          className={`${s.withdrawalActionBtn} ${isCurrentStatus ? s.withdrawalActionBtnActive : s.withdrawalActionBtnInactive}`}
                        >
              ...
              {dashboard && dashboard.withdrawals?.length === 0 && (
                <div className={s.emptyWithdrawals}>
                  ...
                  <p className={s.emptyWithdrawalsText}>No pending withdrawal requests.</p>
                </div>
              )}
            </div>
          </div>
              ...
              <tbody className={s.tbody}>
                {(dashboard?.recentBookings || []).map((booking) => (
                  <tr key={booking._id} className={s.tr}>
                    <td className={s.tdBold}>{booking.userId?.businessName || booking.userId?.name}</td>
                    <td className={s.tdMuted}>{booking.serviceId?.name || 'Service'}</td>
                    <td className={s.tdBold}>{formatMoney(booking.amount)}</td>
                    <td className={s.tdFees}>{formatMoney(booking.platformFeeAmount)}</td>
                    <td className={s.tdEarnings}>{formatMoney(booking.providerPayoutAmount)}</td>
                    <td className={s.td}>
                      <span className={s.bookingPayoutBadge}>{booking.payoutStatus}</span>
                    </td>
                  </tr>
                ))}
                {dashboard && dashboard.recentBookings?.length === 0 && (
                  <tr><td colSpan="6" className={s.emptyTableCell}>No recent transactions.</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <div className={s.confirmModalOverlay}>
          ...
          >
            <div className={s.confirmModalIconRow}>
              ...
              <span className={`${s.withdrawalStatusBadge} ${getWithdrawalStatusClass(pendingWithdrawalAction.status)}`}>
            ...
            <p className={s.confirmModalText}>
              Are you sure you want to mark this withdrawal as {formatStatusLabel(pendingWithdrawalAction.status)}?
            </p>
            <div className={s.confirmModalMeta}>
              <span>{pendingWithdrawal?.userId?.businessName || pendingWithdrawal?.userId?.name || 'Provider'}</span>
