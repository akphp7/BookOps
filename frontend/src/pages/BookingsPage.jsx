...
export default function BookingsPage() {
  ...
  const updateRescheduleDraft = (booking, key, value) => {
    setReschedules((prev) => ({
      ...prev,
      [booking._id]: {
        date: booking.date,
  ...
  // Determine banner variant based on message content
  const getBannerVariant = (msg) =>
    msg.toLowerCase().includes("triggered") || msg.toLowerCase().includes("success")
  ...
  return (
    <AppLayout>
      ...
      <section className={s.headerSection}>
        <div className={s.headerLeftArea}>
          ...
          <div className={s.illustrationContainer}>
            <img src={p7Image} alt="Illustration" className={s.illustrationImg} />
          </div>
        </div>
      ...
      {message && (
        <div
          className={`${s.messageBanner} ${getBannerVariant(message)}`}
        >
              ...
              <div className={s.cardLeftBlock}>
                <div className={s.badgesContainer}>
                  ...
                  {booking.rescheduleCount > 0 && (
                    <span className={`${s.paymentBadgeBase} bg-orange-100 text-orange-700 ml-2`}>
        ...
        <div className={s.modalOverlay}>
          <div className={s.modalContent}>
            <div className={s.modalHeader}>
              ...
              <button onClick={() => setCancelModalBookingId(null)} className={s.modalCloseBtn}>
                <XCircle className="w-5 h-5" />
              </button>
            </div>
            <div className={s.modalBody}>
              <p className={s.modalMessage}>Are you sure you want to cancel this booking? This action cannot be undone, and the customer will be notified.</p>
            </div>
          ...
          <div className={s.modalContent}>
            <div className={s.modalHeader}>
              ...
              <button onClick={() => {
                ...
                setReschedules((prev) => ({ ...prev, [rescheduleModalBooking._id]: undefined }));
            ...
            <div className={s.modalBody}>
              <div className={s.rescheduleGrid}>
                <div className={s.rescheduleInputContainer}>
                  ...
                  <input
                    type="date"
                    value={reschedules[rescheduleModalBooking._id]?.date ?? rescheduleModalBooking.date}
                    onChange={(event) =>
                      updateRescheduleDraft(rescheduleModalBooking, "date", event.target.value)
                    }
                ...
                <div className={s.rescheduleInputContainer}>
                  ...
                  <input
                    type="time"
                    value={reschedules[rescheduleModalBooking._id]?.startTime ?? rescheduleModalBooking.startTime}
                    onChange={(event) =>
                      updateRescheduleDraft(rescheduleModalBooking, "startTime", event.target.value)
                    }
                    className={s.rescheduleInputField}
                  />
                </div>
                <div className={s.rescheduleInputContainer}>
                  ...
                  <input
                    type="time"
                    value={reschedules[rescheduleModalBooking._id]?.endTime ?? rescheduleModalBooking.endTime}
                    onChange={(event) =>
                      updateRescheduleDraft(rescheduleModalBooking, "endTime", event.target.value)
                    }
                    className={s.rescheduleInputField}
                  />
                </div>
