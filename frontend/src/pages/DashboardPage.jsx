...
import {
  ArrowRight, Calendar, CheckCircle, Clock, Wallet, Copy, RefreshCcw
} from "lucide-react";
    ...
    <div className={s.barChartWrapper}>
      <svg viewBox={`0 0 ${width} ${height}`} className={s.barChartSvg}>
        ...
        {data.map((item, index) => {
          ...
          return (
            <g key={item.label}>
              <rect x={x} y={0} width={barWidth} height={height} fill="transparent" className={s.barChartBarInteractive}
                onMouseEnter={() => setTooltip({ x: x + barWidth / 2, y: Math.max(y, padding.top), text: `₹${item.value} earnings on ${item.label}` })}
                onMouseLeave={() => setTooltip(null)} />
              <rect x={x} y={Math.max(y, padding.top)} width={barWidth} height={Math.max(barH, 4)} fill="url(#barGradient)" rx="4" className={`${s.barChartBar} ${tooltip?.x === (x + barWidth / 2) ? 'opacity-100' : 'opacity-80'}`} />
              {showLabel && (
      ...
      {tooltip && (
        <div
          ...
          style={{ left: `${(tooltip.x / width) * 100}%`, top: `calc(${(tooltip.y / height) * 100}% - 4px)` }}
        >
          {tooltip.text}
    ...
    <div className={s.lineChartWrapper}>
      <svg viewBox={`0 0 ${width} ${height}`} className={s.lineChartSvg} role="img" aria-label="Bookings line graph">
        ...
        {points.map((point) => (
          <g key={point.label}>
            <circle
              cx={point.x} cy={point.y} r="18" fill="transparent" className="cursor-pointer"
      ...
      {tooltip && (
        <div
          ...
          style={{ left: `${(tooltip.x / width) * 100}%`, top: `calc(${(tooltip.y / height) * 100}% - 8px)` }}
        >
          {tooltip.text}
  ...
  return (
    <div className={s.statusPanelContainerFlex}>
      <div
        ...
        style={{ background: trueTotal === 0 ? '#f1f5f9' : `conic-gradient(#7D57F5 0% ${confPct}%, #fb923c ${confPct}% ${confPct + reschPct}%, #ef4444 ${confPct + reschPct}% 100%)` }}
      >
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const loadDashboard = async () => {
      ...
      const [meResponse, servicesResponse, bookingsResponse, paymentsResponse] = await Promise.all([
        getMe(), listServices(), listBookings(), getPaymentOverview(),
      ]);
    ...
    if (graphFilter === "daily") {
      ...
      return Array.from({ length: daysInMonth }, (_, i) => {
        ...
        return {
          ...
          value: bookings.filter((b) => {
            ...
            return bd.getFullYear() === year && bd.getMonth() === month && bd.getDate() === (i + 1);
          }).length,
        };
      });
      ...
      return Array.from({ length: 5 }, (_, i) => {
        ...
        return {
          label: `${y}`,
          short: `${y}`,
          value: bookings.filter((b) => getBookingDate(b, today).getFullYear() === y).length,
        };
      });
    }
    return [];
  ...
  const earningTrend = useMemo(() => {
    ...
    const getEarnings = (filteredBookings) => {
      return filteredBookings.reduce((sum, b) => sum + (b.providerPayoutAmount || b.amount || 0), 0) / 100;
    };
    ...
    if (graphFilter === "daily") {
      ...
      return Array.from({ length: daysInMonth }, (_, i) => {
        ...
        return {
          label: dateStr,
          short: `${i + 1}`,
          value: getEarnings(paidBookings.filter((b) => {
            const bd = getBookingDate(b, today);
            return bd.getFullYear() === year && bd.getMonth() === month && bd.getDate() === (i + 1);
          })),
        };
      ...
      return Array.from({ length: 5 }, (_, i) => {
        ...
        return {
          label: `${y}`,
          short: `${y}`,
          value: getEarnings(paidBookings.filter((b) => getBookingDate(b, today).getFullYear() === y)),
        };
      });
    }
    return [];
  ...
  const upcomingBookings = useMemo(() => {
    ...
    return bookings
      ...
      .filter((b) => {
        return b.displayDate >= now && b.status !== "cancelled" && b.status !== "payment_failed";
      })
      ...
      <div className={s.mainContainer}>
          ...
          <div className={s.heroCard}>
            <div className={s.heroContent}>
              <h2 className={s.heroTitle}>
                Grow your practice.<br/>Impact more lives.
              </h2>
              {publicLink ? (
        ...
        <div className={s.statsGrid}>
          {[
            ...
            { label: "Total Income", value: formatMoney(paidBookings.reduce((s, b) => s + (b.providerPayoutAmount || 0), 0)), icon: Wallet, icfg: "text-[#16a34a]", ibg: "bg-[#eafbef]" },
          ].map((stat, i) => (
        ...
        <section className={s.bottomGrid}>
          ...
          <div className={s.topServicesCard}>
            ...
            <div className={s.servicesList}>
              ...
              {topServices.length === 0 && <p className="text-[13px] font-medium text-slate-500 text-center pt-10">Add services to start tracking demand.</p>}
            </div>
          </div>
          ...
          <div className={s.earningsCard}>
            ...
            <div className={s.earningsAmountRow}>
              <h2 className={s.earningsAmount}>{formatMoney(wallet?.available || 0)}</h2>
        ...
        <section className={s.bottomGrid}>
          <div className={s.upcomingCard}>
            ...
            <div className={s.upcomingList}>
              ...
              {upcomingBookings.length === 0 && <p className="text-[13px] font-medium text-slate-500 text-center pt-8">No upcoming bookings.</p>}
            </div>
          </div>
