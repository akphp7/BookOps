...
const dayIcons = [Sun, CalendarDays, CalendarDays, CalendarDays, CalendarDays, CalendarDays, Sun];
...
export default function AvailabilityPage() {
  ...
  const getSlotsForDay = (items, day) => {
  ...
  useEffect(() => {
    const loadInitialAvailability = async () => {
      try {
        ...
        setSlots(getSlotsForDay(items, selectedDay));
      } catch (error) {
        setMessage(
  ...
  const updateSlot = (index, field, value) => {
    ...
    const isOverlapping = slots.some((slot, i) => {
      ...
      return slot.startTime < newSlot.endTime && newSlot.startTime < slot.endTime;
    });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showToast(
        error.response?.data?.message || "Could not save availability", "error"
      );
    } finally {
      setLoading(false);
    }
        ...
        <section>
          <div className={s.leftTopArea}>
            ...
            <div className={s.illustrationContainer}>
              <img src={p5Image} alt="Illustration" className={s.illustrationImg} />
            </div>
          </div>
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    ...
                    setSlots(getSlotsForDay(availability, index));
                  }}
                  className={
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={
                        isSelected ? s.dayIconContainerActive : s.dayIconContainerInactive
                      }
                    >
          ...
          <div className={s.infoBox}>
            ...
            <p className={s.infoBoxText}>
              Customers will only see <span className={s.infoBoxStrong}>available days</span> and times when booking.
            </p>
          </div>
        </section>
