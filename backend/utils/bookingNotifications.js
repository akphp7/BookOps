...
const buildCompanyEmailHtml = ({ title, eyebrow = 'BookMe', intro, rows, accent = '#7D57F5', notes, calendarUrl, footer }) => {
  const detailRows = rows
    ...
    .map((row) => {
      ...
      const rowValue = row.label === 'Verification code'
        ? `<span style="display:inline-block; font-family:Arial, Helvetica, sans-serif; font-size:24px; line-height:1; letter-spacing:6px; color:#1e293b; font-weight:800; white-space:nowrap;">${escapeHtml(value.trim())}</span>`
        : escapeHtml(value);

      return `
      <tr>
        <td style="padding: 14px 0; color: #94a3b8; font-size: 13px; width: 36%; vertical-align: top;">${escapeHtml(row.label)}</td>
        <td style="padding: 14px 0; color: #1e293b; font-size: 14px; font-weight: 700;">${rowValue}</td>
      </tr>
    `;
    })
    .join('');
...
export const sendTestEmail = async (to) => {
  const htmlContent = buildCompanyEmailHtml({
    title: 'BookMe email notifications are working',
    intro: 'This is a test email from BookMe. Booking confirmation, provider alerts, and OTP verification emails will use this sender.',
    rows: [
      { label: 'Provider', value: 'Brevo' },
      { label: 'Status', value: 'Connected' },
    ],
    footer: 'You can now send transactional emails from BookMe.',
  });

  return sendWithBrevo({
    to,
    subject: 'BookMe email notifications are working',
    text: 'This is a test email from BookMe. Booking confirmation, provider alerts, and OTP verification emails will use this sender.',
    htmlContent,
  });
};
